...
おらず、ユーザーのOAuthトークンが必要。Cloudflare Worker から直接呼ぶには
...
```
ポータル(Worker) → /api/registry → (プロキシ+キャッシュ) → GASレジストリ /exec
...
にしてURLを `PROXY_TARGETS` に登録する(URLはWorker secretなので外部に出ない)。
