...
**Zero Trust(Cloudflare Access)偽装の拒否も検証する**。`/api/apps` と `/` に
不正な `Cf-Access-Jwt-Assertion` / `Cf-Access-Authenticated-User-Email` を付けて送り、
それでも `401` / `302` になること(=偽装ヘッダで素通りしないこと)を確認する。本番は
カスタムドメイン、または team domain/aud 未設定の pages.dev エイリアスなので、middleware
のバイパス条件(上記「環境ごとの保護方針」)を満たさず素通りしないのが正しい挙動。
過去に pages.dev エイリアス + 偽装ヘッダで素通りする穴があった(コミット履歴参照)ため、
この回帰をデプロイごとに検知する。

**トリガー**:
- **デプロイ完了**: Cloudflare Pages が発行する GitHub Deployment の
  `deployment_status`(production の success)で発火。
- **継続ヘルスチェック**: `schedule`(30分ごとの cron)。secret ローテ失敗や環境変数
  消失など、デプロイ後に認証が壊れるドリフトを拾う。単なる `/api/health` の 200 監視
  ではなく、`/api/apps` が `401` を返し続けることを監視するのが要点(200監視だと
  「認証が外れた=200」を正常と誤判定してしまう)。
- **手動**: `workflow_dispatch`(URL上書き可)。

**設定**: リポジトリ変数(Settings → Secrets and variables → Actions → **Variables**)
`SMOKE_BASE_URLS` に検査対象をカンマ区切りで入れる。本番カスタムドメインに加え、
...
> 外部の常時監視サービス(UptimeRobot / BetterStack / Checkly 等)を併用してもよい。
> その場合も監視条件は「`/api/apps` が `401`(=認証がかかっている)」にすること。
> なお **Cloudflare 純正の Health Checks はゾーンがアカウント内にある前提**の機能で、
> 本構成(外部DNS + CNAME、ネームサーバ非移管)のカスタムドメインには使えないため、
> 上記の scheduled GitHub Actions を継続ヘルスチェックの主軸にしている。

ローカル実行(任意):

```bash
node scripts/smoke.mjs https://portal.example.co.jp
```
