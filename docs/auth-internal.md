...
認証モデルはホストで違う(カスタムドメイン=middleware が直接ゲート / `*.pages.dev`=前段の
Cloudflare Access がホスト全体をゲート)ため、`scripts/smoke.mjs` はモードとホスト種別で
検査を分ける。production モードは検査対象のホストが `*.pages.dev` かどうかでさらに分岐する。

**① production モード / カスタムドメイン**(例 `portal.example.co.jp`)。前段の Cloudflare
Access は効かない(外部DNSのゾーンはアカウント内に無い)ので middleware が直接応答する。
**正確なステータス** を検証する:
...
**①' production モード / pages.dev ホスト**(例 `<project>.pages.dev`, `<hash>.<project>.pages.dev`)。
本番デプロイに紐づく `*.pages.dev` は **Cloudflare 所有ゾーン**なので、カスタムドメインと違い
前段の Cloudflare Access(Zero Trust)がホスト全体をゲートする。middleware の公開パス
`/api/health` を含め、未認証は Access ログイン(`*.cloudflareaccess.com`)へ 302 されて関数まで
届かないため、**正確なステータスは出ない**。よって②プレビューと同じく「**未認証で `200` を
返さない=何らかの認証でブロックされている**」ことを検証する(`/api/health` も含めて確認し、
Access が前段から外れて公開状態になる事故も検知する)。この分岐が無いと、`/api/health` に
`200` を期待して pages.dev ホストで必ず失敗していた(誤ったアサーション)。

| リクエスト | 期待 | 崩れたときの意味 |
|---|---|---|
| `GET /api/health` | `3xx`/`401`/`403`(≠200) | `200`=Access が前段から外れて公開 |
| `GET /api/apps`(未認証, 偽装ヘッダ有無) | `3xx`/`401`/`403`(≠200) | `200`=台帳データが公開 |
| `GET /api/me`(偽装 `Cf-Access-*` ヘッダ) | `3xx`/`401`/`403`(≠200) | `200`=偽装で素通り |
| `GET /api/proxy/:id`(未認証) | `3xx`/`401`/`403`(≠200) | `200`=GAS中継が公開 |
| `GET /`(未認証, 偽装ヘッダ有無) | `3xx`/`401`/`403`(≠200) | `200`=画面が公開 |
