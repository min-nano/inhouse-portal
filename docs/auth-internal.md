...
| `CF_ACCESS_TEAM_DOMAIN` | 推奨 | 変数 | プレビュー用。Access トークンを**署名検証**する。例 `myteam`(iss を固定)。詳細は下記 |
| `CF_ACCESS_AUD` | 推奨 | 変数 | プレビュー用。Access アプリの AUD タグ(aud を固定) |
...
1. **Cloudflare Access でプレビューを保護**: Zero Trust → Access → Applications で
   ...
   ポリシーを設定する。
2. **署名検証を有効化(推奨)**: Preview 環境の変数に `CF_ACCESS_TEAM_DOMAIN` と
   `CF_ACCESS_AUD`(上記アプリの AUD タグ)を設定する。
